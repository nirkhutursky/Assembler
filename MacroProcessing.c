...
void insert_macro(MacroTable *macro_table, Macro *macro) {
    ...
    if (new_node) {
        new_node->name = strdup(macro->name);
...
void add_line_to_macro(Macro *macro, const char *line_value, int line_number) {
    ...
    if (new_line) {
        new_line->value = strdup(line_value);
        new_line->number = line_number;
        new_line->next = macro->lines; /* Add the new line at the beginning of the list*/
...
void parse_macros(const char *input_file, const char *output_file) {
    ...
    while (read_line(in, line, sizeof(line), &line_number)) {
        if (strncmp(line, "macr", 4) == 0 && !in_macro) {
            handle_macro_definition_start(macro_table, line, macro_name, line_number);
            in_macro = 1;
